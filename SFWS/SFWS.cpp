...
sf::RenderWindow* SFWS::createWindow(const sf::Vector2f& size, const std::string& title_)
{
	weOwnWindow = true;
...
sf::RenderWindow* SFWS::bindToWindow(sf::RenderWindow* window, const std::string& title, SFWSUtil::ResourceSettings settings)
{
	weOwnWindow = false;
...
void SFWS::HandleEvents(const sf::Event& event)
{
...
void SFWS::updateWindowDecorations()
{
...
void SFWS::initialise(SFWSUtil::ResourceSettings settings)
{
	std::cout << "initialiseing Sfws" << std::endl;

	setColours();

	resizerTexture.loadFromFile(settings.resizer);
	...
	titleBarButton.bindToInterface(this, settings);
}
